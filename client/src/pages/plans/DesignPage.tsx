...
`;
  ...
  return (
    <div className="card">
      ...
      <div className="inline-form">
        <label>
          ...
          <select value={selectedApiId} onChange={(e) => handleSelectApi(e.target.value)} disabled={schemaLoading}>
            <option value="">Aucune (schéma non enregistré)</option>
            {apis.map((a) => (
        ...
        <button type="button" className="btn" onClick={handleSaveSchema} disabled={selectedApiId === "" || schemaSaving}>
          {schemaSaving ? "Enregistrement..." : "Enregistrer le schéma"}
        </button>
        <button type="button" className="btn-outline" onClick={handleExportSvg} disabled={exporting || cards.length === 0}>
          {exporting ? "Export en cours..." : "Exporter en SVG"}
        </button>
      </div>
      <div className="inline-form">
        <label>
          Ajouter un matériel
          <select value={addEquipmentId} onChange={(e) => setAddEquipmentId(e.target.value ? Number(e.target.value) : "")}>
        ...
        <button type="button" className="btn" onClick={handleAddCard} disabled={addEquipmentId === ""}>
          Ajouter au plan
        </button>
      </div>
      {pendingPort && (
      ...
      <div className="design-canvas-zoom">
        <button
          type="button"
