...
If `docker compose build` (or `./scripts/run-allinone.ps1`) fails during `npm ci`
...
Fix: export that root CA and drop it here as a `.crt` file; the Dockerfile
picks up every `*.crt` in this directory automatically (via
`update-ca-certificates` + `NODE_EXTRA_CA_CERTS`) — no other change needed.
...
$path = "docker/allinone/certs/zscaler-root-ca.crt"
